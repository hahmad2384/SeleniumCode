package com.class05;

import java.util.List;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;

public class Task4Check {
/*
 * * Check all Years of Experience radio buttons are clickable
* -----------------------------------------------
* Check all Automation Tools checkboxes are clickable
* and keep “Selenium WebDriver” option as selected
 */
	public static void main(String[] args) throws InterruptedException {

		System.setProperty("webdriver.chrome.driver", "/Users/imanahmad/Selenium/chromedriver");
		WebDriver driver=new ChromeDriver();
		driver.manage().window().fullscreen();
		driver.get("https://www.toolsqa.com/automation-practice-form/");
		
		List<WebElement> experList= driver.findElements(By.name("exp"));
		System.out.println(experList.size());
		Thread.sleep(2000);
		
		for(WebElement experience:experList) {
			if(experience.isEnabled()) {
			String value = experience.getAttribute("value");
			System.out.println(value);
			
				experience.click();
				Thread.sleep(1000);
			
			}
		}
		String boxToSelect = "Selenium Webdriver";

		List<WebElement> toolList = driver.findElements(By.name("tool"));
		System.out.println(toolList.size());
		Thread.sleep(2000);
		
		for(WebElement tools:toolList) {
			String value = tools.getAttribute("value");
			System.out.println(value);
			
			if(tools.isEnabled()) {
				tools.click();
				Thread.sleep(2000);
			if(value.equals(boxToSelect)) {
				tools.click();
				Thread.sleep(2000);
			}
			}
		}

		Thread.sleep(2000);
		driver.quit();
	}

}
